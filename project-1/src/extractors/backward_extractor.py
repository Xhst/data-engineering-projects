...
def extract_table_caption(
    table: html.HtmlElement, usedCaptions: set[str]
) -> tuple[str, str]:
...
def extract_table_footnotes(table: html.HtmlElement, table_id: str) -> list[str]:
    ...
    footnotes_xpath = table_container.xpath(
...
def extract_table_references(
    paper: html.HtmlElement, table_id: str, tableDenomination: str
) -> list[str]:
    result = []
    ...
    if a_refs:
        for a_ref in a_refs:
            text_ref_content = (
                a_ref.text_content().strip()
                if hasattr(a_ref, "text_content")
                else a_ref.strip()
            )
    ...
    # if no references are found, try to find them by looking for text references
        ...
        for text_ref in text_refs:
            text_ref_content = (
                text_ref.text_content().strip()
                if hasattr(text_ref, "text_content")
                else text_ref.strip()
            )
...
def extract_paper_data(paper: html.HtmlElement) -> dict[str, TableData]:
    ...
    tables: list[html.HtmlElement] = paper.xpath(
        '//table[contains(@class, "ltx_tabular") and not(ancestor::table)]'
    )
    ...
    for table in tables:
        ...
        tableData.caption, tableDenomination = extract_table_caption(
            table, usedCaptions
        )
        ...
        tableData.references = extract_table_references(
            paper, table_id, tableDenomination
        )
