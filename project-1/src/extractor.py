...
def process_file(
    filename: str, source_folder: str, extract_folder: str, extractor: str
):
    with open(f"{source_folder}/{filename}", "r", encoding="utf-8") as htmlFile:
        ...
        with open(
            f"{extract_folder}/{filename}.json", "w", encoding="utf-8"
        ) as jsonFile:
...
if __name__ == "__main__":
    ...
    extractors = {
        "forward_extractor": forward_extractor,
        "backward_extractor": backward_extractor,
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for folder in folders_to_extract:
            ...
            for future in tqdm(
                ...
                desc=f"Processing {source_folder}",
                unit=" file",
                colour="green",
                total=len(futures),
            ):
